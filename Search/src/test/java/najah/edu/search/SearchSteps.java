	...
	public void i_search_about_home_by(String spec) {
		...
		if(spec.equals(Hometype.APARTMENT.toString())||spec.equals(Hometype.HOUSE.toString())) {
		     ...
		     for(String d:typelist) {
				...
				result=search.byType(spec,typelist,count,keys,dataTable);}
			...
			spec.equals(Homematerial.WOOD.toString())) {
		{
			...
			result=search.byMaterial(spec,materiallist,count,keys,dataTable);}
	}
			...
			for(String d:placementlist) {
						...
						result=search.byPlacement(spec,placementlist,count,keys,dataTable);}
		     ...
		     for(String d:petslist) {
						...
						result=search. bypets(spec,petslist,count,keys,dataTable);
						 }
...
this.price=price;
		count=0;
			...
			r=search.bybellowprice(price,pricelist,count,keys,values,dataTable);
			//System.out.print(r);
	...
	this.area=area;
		...
		r=search.bybellowarea(area,arealist,count,keys,values,dataTable);
		//System.out.print(r);
...
//count=0;
			...
			r=search.bybetweenprice(lp,hp,pricelist,count,keys,values,dataTable);
			//System.out.print(r);
	...
	public void i_search_about_home_with_area_between_and(Integer la, Integer ha) {
	    count=0;
			...
			r=search.bybetweenarea(la,ha,arealist,keys,values,dataTable);
			//System.out.print(r);
	...
	public  void iSearchAboutHomeByBedrooms(Integer num_of_bedroom) {
	    // count=0;
				...
				if((Integer.parseInt(d))== num_of_bedroom) {count++;
					...
					r=search.bynumbedrooms(num_of_bedroom,numofbedroomslist,count,keys,values,dataTable);
	...
	public void iSearchAboutHomeByBathrooms(Integer num_of_bathroom) {
	    count=0;
					...
					if((Integer.parseInt(d))== num_of_bathroom) {count++;
						...
						r=search. bynumbathroom(num_of_bathroom,numofbathroomslist,count,keys,values,dataTable);
	}
...
if(amenities.equals(Amenities.AIRCONDITIONING.toString())||amenities.equals(Amenities.BALCONY.toString())||amenities.equals(Amenities.ELEVATOR.toString())||
		...
		result=search.byamenities(amenities, amenitieslist, count, keys, values, dataTable);}
		...
		public void iSearchAboutHomeByLeaseLength(Integer int1) {
		    count=0;
						...
						if((Integer.parseInt(d))== int1) {count++;
							...
							r=search.byleaseleng(int1,leaseLengthlist,count,keys,values,dataTable);
